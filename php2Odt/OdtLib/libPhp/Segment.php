...
class Segment {
    ...
    public function __construct($name, $odf, $xml) {
        ...
        $this->dom = new DOMDocument; // create new DOMDocument instance
    ...
    public function analyseChildren($string = '[\S]*') {
        ...
        foreach ($this->elements as $key => $element) {
            ...
            if (preg_match($reg, $element->textContent)) {
                preg_match_all($reg, $element->textContent, $this->_childs[$i++]/* $this->_matches[$i++] */);
            }
        }
    ...
    public function setvar($key, $value) {
        ...
        if (is_array($value)) {
            ...
            foreach ($value as $v) {
                foreach ($this->dom->getElementsByTagNameNS('urn:oasis:names:tc:opendocument:xmlns:text:1.0', 'p') as $element) {
                    // if ($element->getAttributeNS('urn:oasis:names:tc:opendocument:xmlns:text:1.0', 'style-name') == $this->name)
